...
def test_reader_opk_ubupath():
    ...
    obj = read_opk(INPUT_OPK_UBU, {"interval": [2, None],"header": "N X Y Z O P K C","unit_angle": UNIT_ANGLE,"linear_alteration":True}, work)
...
def test_reader_opk_winpath():
    ...
    if platform.system() in ['Linux','Darwin']:
        with pytest.raises(FileNotFoundError) as e_info:
            obj = read_opk(INPUT_OPK_WIN, {"interval": [2, None],"header": "N X Y Z O P K C","unit_angle": UNIT_ANGLE,"linear_alteration":True}, work)
    else:
        obj = read_opk(INPUT_OPK_WIN, {"interval": [2, None],"header": "N X Y Z O P K C","unit_angle": UNIT_ANGLE,"linear_alteration":True}, work)
        assert obj.name == "23FD1305_alt_test"
        assert obj.shots["23FD1305x00026_01306"].name_shot == "23FD1305x00026_01306"
...
def test_reader_opk_bad_header_add_column():
    ...
    with pytest.raises(ValueError) as e_info:
        obj = read_opk(INPUT_OPK_UBU, {"interval": [2, None],"header": "N X Y Z O P K C S","unit_angle": UNIT_ANGLE,"linear_alteration":True}, work)


def test_reader_opk_bad_header_miss_column():
    work = Worksite("23FD1305_alt_test")
    with pytest.raises(ValueError) as e_info:
        obj = read_opk(INPUT_OPK_UBU, {"interval": [2, None],"header": "N X Y Z O P K","unit_angle": UNIT_ANGLE,"linear_alteration":True}, work)


def test_reader_opk_diffheader():
    work = Worksite("23FD1305_alt_test")
    obj = read_opk("test/data/dataset1/23FD1305_alt_NCPOKXYZ.OPK", {"interval": [2, None],"header": "S N C P O K X Y Z","unit_angle": UNIT_ANGLE,"linear_alteration":True}, work)
    assert obj.name == "23FD1305_alt_test"
    assert obj.shots["23FD1305x00026_01306"].name_shot == "23FD1305x00026_01306"
    assert obj.shots["23FD1305x00026_01306"].pos_shot[0] == 814975.925
    assert obj.shots["23FD1305x00026_01306"].pos_shot[1] == 6283986.148
