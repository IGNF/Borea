...
def test_world_to_image():
    ...
    actual = WorldImageShot(shot).world_to_image(point_terrain[0], point_terrain[1], point_terrain[2], cam, DATA_TYPE_Z, SHOT_TYPE_Z)
...
def test_world_to_image_withoutgeoid():
    ...
    with pytest.raises(ValueError) as e_info:
        z_nadir = ImageWorldShot(shot).image_to_world(cam.ppax, cam.ppay, cam, 'height', 'height', False)[2]
        shot.set_z_nadir(z_nadir)
        WorldImageShot(shot).world_to_image(point_terrain[0], point_terrain[1], point_terrain[2], cam, DATA_TYPE_Z, SHOT_TYPE_Z)
...
def test_world_to_image_sametypea():
    ...
    z_nadir = ImageWorldShot(shot).image_to_world(cam.ppax, cam.ppay, cam, 'height', 'height', False)[2]
    shot.set_z_nadir(z_nadir)
    WorldImageShot(shot).world_to_image(point_terrain[0], point_terrain[1], point_terrain[2], cam, 'altitude', 'altitude')
