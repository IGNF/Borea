...
def test_image_to_world():
    ...
    actual = ImageWorldShot(shot).image_to_world(point_image[0], point_image[1], cam, DATA_TYPE_Z, SHOT_TYPE_Z)
...
def test_image_to_world_sametype_withoutgeoid():
    ...
    z_nadir = ImageWorldShot(shot).image_to_world(cam.ppax, cam.ppay, cam, 'height', 'height', False)[2]
    shot.set_z_nadir(z_nadir)
    actual = ImageWorldShot(shot).image_to_world(point_image[0], point_image[1], cam, DATA_TYPE_Z, DATA_TYPE_Z)
...
def test_image_to_world_multipoint():
    ...
    z_nadir = ImageWorldShot(shot).image_to_world(cam.ppax, cam.ppay, cam, 'height', 'height', False)[2]
    shot.set_z_nadir(z_nadir)
    actual = ImageWorldShot(shot).image_to_world(c, l, cam, DATA_TYPE_Z, SHOT_TYPE_Z)
