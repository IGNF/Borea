...
def test_from_shot_eucli():
    ...
    shot_eucli = Shot.from_param_euclidean("test_shot", pos_shot_eucli, mat_rot_eucli, "test_cam", "degree",True, "opk", False)
...
def test_set_unit_angle_degree():
    shot = Shot("test_shot", np.array([814975.925, 6283986.148,1771.280]), np.array([np.pi,0,2*np.pi]), "test_cam", 'radian',True,"opk")
...
def test_set_unit_angle_radian():
    shot = Shot("test_shot", np.array([814975.925, 6283986.148,1771.280]), np.array([180,0,360]), "test_cam", 'degree',True,"opk")
